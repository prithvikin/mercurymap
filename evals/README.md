...
- **Coordinate/place agreement** — both coordinates must be within one degree of a reference coordinate for the returned place/country. The reference source differs by mode:
  ...
  - **Live mode** uses OpenCage, the same geocoder the app itself calls from `LocationSearch`. This is required, because the gazetteer holds 41 places while the endpoints exist to recommend from the whole world — the first live pass turned 27 correct answers into regressions while catching zero real coordinate errors. Results are cached in `evals/results/geocode-cache.json` between runs; the cache carries a version that is bumped whenever resolution logic changes, so stale entries are discarded rather than masking a fix. A result that collapses to the country itself (OpenCage answers an unmatched query with a country centroid rather than an empty result) is treated as unresolved, and a trailing generic designator is retried without it, so `Torres del Paine National Park` resolves via `Torres del Paine`.

  Either way, a place the reference source cannot resolve fails closed as unverifiable rather than being accepted because its numbers are in range: a place no geocoder knows is precisely the hallucination this check exists to catch.
- **Single place per suggestion** — a suggestion becomes exactly one map pin, so `place` must name exactly one destination. Commas, `and`, `&`, and the country name appearing alongside other text are all rejected. This came out of the first live pass, which returned `"Hiroshima and Miyajima"`, `"Stone Town, Zanzibar, Tanzania"`, and `"Kyoto Prefecture's Miyama & the Kumano Kodo, Wakayama, Japan"` — each pinning one coordinate for several places. City-states are exempt, so `Singapore`/`Singapore` is accepted.
