...
-- the owner uploads to the PUBLIC map.
--
...
-- rejected. The same asymmetry left the owner unable to edit or delete public
-- rows. Note that the SELECT side already had an explicit `user_id IS NULL`
-- branch (schema.sql:27-28); only the write side was missing one.
--
...
-- ---------------------------------------------------------------------------
-- Do NOT simplify these to `WITH CHECK (user_id IS NULL)`.
--
...
-- frontend/src/lib/supabase.ts). A bare `user_id IS NULL` check would let
-- anyone on the internet write arbitrary rows onto the public map with a
-- two-line fetch -- unmoderated content on the front page of the site.
--
-- The `auth.uid() = <owner>` conjunct is the whole security boundary here. It
-- must not be dropped, including "just to let signed-out visitors contribute".
--
...
BEGIN
  ...
  -- These sit alongside the per-user policies from schema.sql rather than
  -- replacing them: policies are permissive and OR-ed, so a normal user's
  -- private uploads keep working through "Users can insert their own photos".

  EXECUTE 'DROP POLICY IF EXISTS "Owner can insert public photos" ON photos';
  EXECUTE format(
    'CREATE POLICY "Owner can insert public photos" ON photos
       FOR INSERT WITH CHECK (user_id IS NULL AND auth.uid() = %L::uuid)',
    owner_id
  );
...
-- Verify
-- ---------------------------------------------------------------------------
-- Expect the three "Owner can ... public photos" policies below, alongside the
-- per-user ones from schema.sql. Also check the SELECT rows while you are
-- here: if any has a `qual` of just `true`, the stale policy from
-- add_auth_to_existing.sql survived and every user's private photos are
-- readable with the anon key -- run add_recommendations.sql to fix that.
--
